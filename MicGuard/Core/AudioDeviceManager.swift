...
class AudioDeviceManager: AudioDeviceManaging {
    ...
    func isDeviceRunning(_ device: AudioDevice) -> Bool {
        // Method 1: Check if device is running somewhere (global scope)
        ...
        // Method 2: Check if device is running with input scope (more reliable for browsers)
        ...
        let runningSomewhere = (status1 == noErr && isRunningSomewhere != 0)
        let runningInput = (status2 == noErr && isRunningInput != 0)

        return runningSomewhere || runningInput
    }
